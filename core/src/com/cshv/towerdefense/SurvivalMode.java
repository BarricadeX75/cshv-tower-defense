...
public class SurvivalMode extends GameScreen {

    private final TowerDefenseGame towerDefenseGame;
    private Label labelWave;

    public SurvivalMode (TowerDefenseGame towerDefenseGame, Player player) {
    ...
    public void show() {
        ...
        Timer.schedule(setWave,1,25+nbMonster);
    ...
    public void checkGame(){
        if(!win && !lose) {
            if (_player.getVieCombat() == 0) {
                ...
                int goldWin = (int) (20 * Math.pow(1.15, lvlStage - 1));
                goldWin = goldWin * numWave;
                gold += goldWin;
